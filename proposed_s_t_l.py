...
def load_and_process_data(filename: str):
    ...
    features = np.stack([trend, seasonal, resid], axis=1)
    return features, data  # Features(입력), Original Data(정답 검증용)
...
# 타겟(정답) 스케일링 - Trend+Seasonal+Resid 합이 원본과 같으므로 원본 스케일링
...
# 모델 초기화
model = STL_Attention_LSTM(input_dim=3, hidden_dim=HIDDEN_DIM, layer_dim=LAYER_DIM, output_dim=PREDICT_HORIZON)
